...
export default function AdminDashboard() {
  ...
  const downloadCSV = () => {
    ...
    const headers = ["Name", "Email", "Mobile", "LeetCode Handle", "UID", "Department", "Year", "Total Solved", "LC Rating", "Current Streak"];
    const rows = profiles.map(p => [p.name, p.email, p.mobile, p.leetcode_handle, p.uid, p.department, p.year, p.total_solved || 0, p.lc_rating || 0, p.current_streak || 0]);
                ...
                <TableBody>
                  {profiles.map(p => (
                    <TableRow key={p.id} className="hover:bg-slate-50 border-b border-slate-100">
                      ...
                      <TableCell className="text-center font-mono text-primary text-xs font-black">{p.total_solved}</TableCell>
                      <TableCell className="text-right font-mono text-xs font-bold">{p.lc_rating > 0 ? p.lc_rating : "1500"}</TableCell>
                    </TableRow>
                  ))}
                </TableBody>
              </Table>
