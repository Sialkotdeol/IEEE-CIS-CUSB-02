...
export default function LeaderboardPage() {
  ...
  useEffect(() => {
    const fetchProfiles = async () => {
      try {
        const res = await fetch("/api/code-warriors/profile", { cache: 'no-store' });
        if (res.ok) {
          ...
          if (data.profiles && data.profiles.length > 0) {
            ...
            setProfiles([
              { id: "1", name: "Neal Wu", leetcode_handle: "neal_wu", department: "Computer Science", year: "3rd Year", current_streak: 28, quests_solved: 94, cw_rating: 940 },
              { id: "2", name: "Alex Rivera", leetcode_handle: "alex_rivera", department: "AI & ML", year: "3rd Year", current_streak: 21, quests_solved: 76, cw_rating: 760 },
              { id: "3", name: "Gennady K", leetcode_handle: "tourist", department: "Software Eng", year: "4th Year", current_streak: 18, quests_solved: 68, cw_rating: 680 },
              { id: "4", name: "Priya Sharma", leetcode_handle: "priya_codes", department: "Computer Science", year: "2nd Year", current_streak: 14, quests_solved: 48, cw_rating: 480 },
              { id: "5", name: "Rohan Verma", leetcode_handle: "rohan_v", department: "Information Tech", year: "3rd Year", current_streak: 12, quests_solved: 39, cw_rating: 390 },
            ]);
          }
        }
      } catch (err) {
  ...
  const processedProfiles = profiles
    ...
    .sort((a, b) => {
      if (sortBy === "streak") {
        if ((b.current_streak || 0) !== (a.current_streak || 0)) return (b.current_streak || 0) - (a.current_streak || 0);
        return (b.cw_rating || 0) - (a.cw_rating || 0);
      }
      if (sortBy === "solved") {
        if ((b.quests_solved || 0) !== (a.quests_solved || 0)) return (b.quests_solved || 0) - (a.quests_solved || 0);
        return (b.cw_rating || 0) - (a.cw_rating || 0);
      }
      if (sortBy === "rating") {
        if ((b.cw_rating || 0) !== (a.cw_rating || 0)) return (b.cw_rating || 0) - (a.cw_rating || 0);
        if ((b.quests_solved || 0) !== (a.quests_solved || 0)) return (b.quests_solved || 0) - (a.quests_solved || 0);
        ...
        <div className="grid grid-cols-1 md:grid-cols-3 gap-6 items-end pt-4 max-w-4xl mx-auto mb-4">
          ...
          <SpotlightCard glowHue={205} spotSize={220} borderSize={2} className="bg-white border border-slate-200 rounded-3xl p-6 text-center order-2 md:order-1 h-[240px] flex flex-col justify-between shadow-md">
            ...
            <div className="mt-4 pt-4 border-t border-slate-100 flex justify-around text-xs">
              ...
              <div>
                ...
                <p className="text-base font-black text-primary mt-0.5">{processedProfiles[1].quests_solved} ⚔️</p>
              </div>
            </div>
          </SpotlightCard>
          ...
          <SpotlightCard glowHue={205} spotSize={260} borderSize={2} className="bg-gradient-to-b from-amber-50/80 via-white to-white border-2 border-amber-300 shadow-xl rounded-3xl p-6 text-center order-1 md:order-2 h-[270px] flex flex-col justify-between relative overflow-hidden">
            ...
            <div className="mt-4 pt-4 border-t border-amber-200/60 flex justify-around text-xs">
              ...
              <div>
                ...
                <p className="text-lg font-black text-primary mt-0.5">{processedProfiles[0].quests_solved} ⚔️</p>
              </div>
            </div>
          </SpotlightCard>
          ...
          <SpotlightCard glowHue={205} spotSize={220} borderSize={2} className="bg-white border border-slate-200 rounded-3xl p-6 text-center order-3 h-[220px] flex flex-col justify-between shadow-md">
            ...
            <div className="mt-4 pt-4 border-t border-slate-100 flex justify-around text-xs">
              ...
              <div>
                <p className="text-slate-400 font-mono text-[10px] uppercase font-bold">Solved</p>
                <p className="text-sm font-black text-primary mt-0.5">{processedProfiles[2].quests_solved} ⚔️</p>
              </div>
            </div>
          </SpotlightCard>
                  ...
                  return (
                    ...
                    >
                      ...
                      <TableCell className="text-right font-black text-sm pr-6">
                        <span className="font-mono">{p.cw_rating > 0 ? p.cw_rating : "Unrated"}</span>
                      </TableCell>
                    </TableRow>
                  );
                })
